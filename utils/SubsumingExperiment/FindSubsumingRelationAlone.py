...
class MutantSet(object):
        ...
        for root, dirnames, filenames in os.walk(self.globalPath):
            for filename in fnmatch.filter(filenames, "*.java"):
                if str(filename) == "original.java":
                    continue
                newMutant = Mutant()
                newMutant.path = os.path.relpath(
                    os.path.join(root, filename), self.globalPath
                )
    ...
    def retrieveFailedTestResults(self):
        for mutant in self.mutants:
            ...
            with open(
                os.path.join(mutant.globalPath, mutant.getBuildResultFilePath()), "rU"
            ) as buildResultFileHandle:
    ...
    def assignStatus(self):
        ...
        for mutant1 in self.mutants:
            for mutant2 in self.mutants:
                ...
                if (
                    mutant1 is mutant2
                    or len(mutant1.failedTests) == 0
                    or len(mutant2.failedTests) == 0
                ):
                    continue
        ...
        for mutant in self.mutants:
            assert isinstance(mutant, Mutant)
            if (
                len(mutant.subsumedby - mutant.mutuallySubsuming) == 0
                and len(mutant.failedTests) > 0
            ):
...
class MutantSubsumptionGraphEdge(object):
    ...
    def __eq__(self, other):
        if (
            self.mutantSubsuming is other.mutantSubsuming
            and self.mutantSubsumed is other.mutantSubsumed
        ):
            return True
        return False
...
if __name__ == "__main__":
    ...
    for mutant in fSet.mutants:
        ...
        mutuallySubsumingMutants += (
            1
            if mutant.isMutuallySubsuming() is True and mutant.isSubsuming is True
            else 0
        )
        msGroup = set(mutant.mutuallySubsuming)
        msGroup.update({mutant})
    ...
    print(
        "Killed Mutants:                                 ",
        killedMutants,
        "\t{:.2f}%".format(float(killedMutants) * 100.0 / totalMutants),
    )
    print(
        "Subsuming Mutants:                              ",
        subsumingMutants,
        "\t{:.2f}%".format(float(subsumingMutants) * 100.0 / totalMutants),
    )
    print(
        "Mutually Subsuming Mutants:                     ",
        mutuallySubsumingMutants,
        "\t{:.2f}%".format(float(mutuallySubsumingMutants) * 100.0 / totalMutants),
    )
