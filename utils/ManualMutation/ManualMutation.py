...
class CodeFile(object):
    ...
    def readContents(self):
        ...
        self.contents = str(
            unicodedata.normalize("NFKD", fileData).encode("ascii", "replace")
        )
    ...
    def __sub__(self, other):
        ...
        if abs(len(self.strippedContents) - len(other.strippedContents)) > max(
            0.05 * (len(self.strippedContents) + len(other.strippedContents)), 50
        ):
            return 99999
...
class ManualMutation(object):
    def __init__(self, sourcePath=None, mutantsPath=None):
        ...
        self.targetPath = os.path.abspath(
            os.path.join(self.sourcePath, os.path.pardir, "mutated")
        )
    ...
    def groupMutants(self):
        ...
        for sourceFile in self.sourceFiles:
            ...
            for mutantFile in self.mutantFiles:
                ...
                sourceFile.distanceFrom[mutantFile] = mutantFile.distanceFrom[
                    sourceFile
                ]
                i += 1
                sys.stdout.write(
                    ...
                    + "/"
                    + str(totalOperations)
                    + " ({0:.2f}%)".format((100.0 * i) / totalOperations)
                    + "       \r"
                )
                sys.stdout.flush()
        ...
        for mutantFile in self.mutantFiles:
            ...
            for sourceFile in mutantFile.distanceFrom.keys():
                tmpMin = (
                    sourceFile
                    if tmpMin is None
                    or mutantFile.distanceFrom[sourceFile]
                    < mutantFile.distanceFrom[tmpMin]
                    else tmpMin
                )
    ...
    def createSingleMutant(self, sourceFile, mutantFile):
        ...
        if not os.path.isfile(os.path.join(mutantDir, "original.java")):
            shutil.copyfile(
                sourceFile.fullPath, os.path.join(mutantDir, "original.java")
            )
        ...
        generatedMutantFile = os.path.abspath(
            os.path.join(mutantDir, str(counter) + ".java")
        )
